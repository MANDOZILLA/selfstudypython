...
import { diagnosticSuite } from "./diagnostic-suites.js";
export async function runSubmission(request, loadRuntime) {
  ...
  try {
    ...
    const suites = { "payments-csv-v1": paymentsSuite, "api-normalization-v1": apiSuite, "contacts-v1": contactsSuite, "inventory-v1": inventorySuite, "csv-tags-v1": csvTagsSuite, "invoice-cents-v1": invoiceCentsSuite, "webhook-events-v1": webhookEventsSuite, "settlement-reconciliation-v1": settlementReconciliationSuite };
    const raw = JSON.parse(await runtime.runPythonAsync(suites[request.graderId] ?? diagnosticSuite(request.graderId), { globals: namespace }));
