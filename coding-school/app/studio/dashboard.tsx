...
import { BaselineEntry } from "./diagnostic";
import { currentDiagnostic } from "../../lib/diagnostic";
...
function Today({ studio }: { studio: Studio }) {
  ...
  const baseline=currentDiagnostic(studio.state);
  const needsBaseline=!model.run && !studio.state.missionRuns.length && baseline?.status!=="completed";
  return <>
    ...
    {needsBaseline ? <BaselineEntry studio={studio}/> : available && model.mission ? <section className="mission-document" aria-labelledby="mission-title">
    ...
    {baseline?.status==="completed" && <section className="document"><h2>Why this mission</h2><p>{model.selection.reason??"Resume your active mission before starting another."}</p><button className="text-button" onClick={()=>studio.beginBaseline()}>View observed skills and prerequisites →</button></section>}
    ...
    <p className="page-footnote">Your work is saved on this computer in SQLite. Reading, practice, and independent evidence are recorded separately.</p>
  </>;
}
